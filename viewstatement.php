...
<body>
	<center>
	<form method="POST" action ="viewstatement.php">
	<table width='50%'>
		<td>
      <fieldset>
              ...
              <?php
              $ssn = $_COOKIE["ssn"];
              $sql = "SELECT ssn, a.account_number, type from account a, has_account h where h.account_number=a.account_number and h.ssn = '$ssn';";
            	$result = $conn->query($sql);

            	while ($row = $result->fetch_assoc()) {
                $account_number  = $row['account_number'];
                $type  = $row['type'];
                echo "<option value='$account_number'>$account_number-$type</option>";
            	}
              ?>
