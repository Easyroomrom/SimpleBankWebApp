...
<body>
	<center>
	<form method="POST" action ="transferout.php">
	<table width='50%'>
		<td>
      <fieldset>
        <h3>Transfer money to a foreign account.</h3>

        <?php
        $account_number_rec  = $_POST['account_number_rec'];
        $routing_number_rec  = $_POST['routing_number_rec'];
        ...
        if ($account_number_rec == $account_number_send && $account_number_send != "") {
          ...
          if ($type == "savings" && $widthdrawals >= 6) {
            ...
            $sql = "INSERT into transactions_account (account_number,t_id,amount,type,date,description) values ('$account_number_send',NULL,'$amount','debit',CURDATE(),'Transfer out to $account_number_rec');";
            $conn->query($sql);
        ...
        ?>
              ...
              ?>
					  ...
					  <BR><BR>
          <label>Receiving Account Number (12 characters):  </label><input required pattern=".{12,12}" type='text' name="account_number_rec" id="account_number_rec" maxlength="12">

          <label>Receiving Routing Number (9 characters):  </label><input required pattern=".{9,9}" type='text' name="routing_number_rec" id="routing_number_rec" maxlength="9">
  					<BR><BR>
